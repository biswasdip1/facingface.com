          ...
          <div className="px-5 pb-6 sm:px-7 sm:pb-7">
            ...
            {editing ? (
              ...
              <>
                ...
                {user.bio && <p className="text-base text-muted-foreground leading-relaxed mb-3">{user.bio}</p>}
                <div className="space-y-0.5 mt-1">
                  {user.currentRole && <p className="text-base font-semibold text-foreground">{user.currentRole}</p>}
                  {user.hometown && <p className="text-sm text-muted-foreground">From: {user.hometown}</p>}
                  {user.currentLocation && <p className="text-sm text-muted-foreground">Lives in: {user.currentLocation}</p>}
                  {birthDayMonth && <p className="text-sm text-muted-foreground">Birthday: {birthDayMonth}</p>}
                  {isOwnProfile && <button type="button" onClick={openBirthdayDialog} className="mt-1 inline-flex items-center gap-1.5 text-sm font-bold text-[var(--its-red)] hover:underline"><Gift size={13} />{birthDayMonth ? "Edit birthday" : "Add birthday"}</button>}
                  {user.hobby && <p className="text-sm text-muted-foreground">Hobby: {user.hobby}</p>}
                  {user.website && (
                    <a
                      href={user.website.startsWith('http') ? user.website : `https://${user.website}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-1.5 text-sm text-[var(--its-red)] hover:underline truncate mt-1"
                    >
                  ...
                  {user.youtubeChannel && <a href={user.youtubeChannel.startsWith('http') ? user.youtubeChannel : `https://${user.youtubeChannel}`} target="_blank" rel="noopener noreferrer" className="text-sm text-[var(--its-red)] hover:underline block">YouTube Channel</a>}
                </div>
              </>
            )}
            ...
            <div className="grid grid-cols-3 gap-2 mt-5">
              {[
                ...
                <div key={stat.label} className="text-center bg-muted/40 border border-border/60 rounded-md py-2 px-1 shadow-sm">
                  ...
                  <p className="mt-1 text-[11px] font-black uppercase tracking-[0.18em] text-muted-foreground">{stat.label}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            {inlineBioEditing && isOwnProfile ? (
              ...
              user.bio && (
                <div className="group relative mb-3">
                  <p className="text-base text-foreground leading-relaxed pr-6">{user.bio}</p>
                  {isOwnProfile && (
                    <button
