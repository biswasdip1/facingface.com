-- Additional Sales & Buy PostgreSQL repair for migrated MySQL databases.
-- This migration is intentionally idempotent. It covers live databases where 0007 did not
-- run, only partially ran, or encountered a schema shape not covered by the first repair.
...
--> statement-breakpoint
ALTER TABLE "shop_listings" ADD COLUMN IF NOT EXISTS "title" varchar(255);
--> statement-breakpoint
ALTER TABLE "shop_listings" ADD COLUMN IF NOT EXISTS "description" text;
--> statement-breakpoint
ALTER TABLE "shop_listings" ADD COLUMN IF NOT EXISTS "price" numeric(12, 2);
--> statement-breakpoint
ALTER TABLE "shop_listings" ADD COLUMN IF NOT EXISTS "currency" varchar(10) DEFAULT 'USD';
--> statement-breakpoint
ALTER TABLE "shop_listings" ADD COLUMN IF NOT EXISTS "condition" shop_listings_condition_enum DEFAULT 'good';
--> statement-breakpoint
...
--> statement-breakpoint
ALTER TABLE "shop_listings" ADD COLUMN IF NOT EXISTS "status" shop_listings_status_enum DEFAULT 'active';
--> statement-breakpoint
...
--> statement-breakpoint
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'shop_listings' AND column_name = 'sellerid') THEN
    UPDATE "shop_listings" SET "sellerId" = "sellerid" WHERE "sellerId" IS NULL;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'shop_listings' AND column_name = 'mediaurls') THEN
    UPDATE "shop_listings" SET "mediaUrls" = "mediaurls" WHERE "mediaUrls" IS NULL;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'shop_listings' AND column_name = 'contactemail') THEN
    UPDATE "shop_listings" SET "contactEmail" = "contactemail" WHERE "contactEmail" IS NULL;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'shop_listings' AND column_name = 'contactphone') THEN
    UPDATE "shop_listings" SET "contactPhone" = "contactphone" WHERE "contactPhone" IS NULL;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'shop_listings' AND column_name = 'isflagged') THEN
    UPDATE "shop_listings" SET "isFlagged" = "isflagged" WHERE "isFlagged" IS NULL;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'shop_listings' AND column_name = 'flagreason') THEN
    UPDATE "shop_listings" SET "flagReason" = "flagreason" WHERE "flagReason" IS NULL;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'shop_listings' AND column_name = 'removedbyadminid') THEN
    UPDATE "shop_listings" SET "removedByAdminId" = "removedbyadminid" WHERE "removedByAdminId" IS NULL;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'shop_listings' AND column_name = 'viewcount') THEN
    UPDATE "shop_listings" SET "viewCount" = "viewcount" WHERE "viewCount" IS NULL;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'shop_listings' AND column_name = 'createdat') THEN
    UPDATE "shop_listings" SET "createdAt" = "createdat" WHERE "createdAt" IS NULL;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'shop_listings' AND column_name = 'updatedat') THEN
    UPDATE "shop_listings" SET "updatedAt" = "updatedat" WHERE "updatedAt" IS NULL;
  END IF;
END $$;
--> statement-breakpoint
UPDATE "shop_listings" SET "sellerId" = 0 WHERE "sellerId" IS NULL;
--> statement-breakpoint
UPDATE "shop_listings" SET "title" = 'Untitled listing' WHERE "title" IS NULL OR trim("title") = '';
--> statement-breakpoint
UPDATE "shop_listings" SET "price" = 0 WHERE "price" IS NULL;
--> statement-breakpoint
UPDATE "shop_listings" SET "currency" = 'USD' WHERE "currency" IS NULL OR trim("currency") = '';
--> statement-breakpoint
UPDATE "shop_listings" SET "condition" = 'good' WHERE "condition" IS NULL;
--> statement-breakpoint
UPDATE "shop_listings" SET "category" = 'other' WHERE "category" IS NULL OR trim("category") = '';
--> statement-breakpoint
UPDATE "shop_listings" SET "mediaUrls" = '[]'::json WHERE "mediaUrls" IS NULL;
--> statement-breakpoint
UPDATE "shop_listings" SET "status" = 'active' WHERE "status" IS NULL;
--> statement-breakpoint
...
--> statement-breakpoint
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'shop_listings' AND column_name = 'condition') THEN
    ALTER TABLE "shop_listings"
      ALTER COLUMN "condition" DROP DEFAULT,
      ALTER COLUMN "condition" TYPE shop_listings_condition_enum
      USING CASE
        WHEN "condition"::text IN ('new', 'like_new', 'good', 'fair', 'for_parts') THEN "condition"::text::shop_listings_condition_enum
        ELSE 'good'::shop_listings_condition_enum
      END,
...
--> statement-breakpoint
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'shop_listings' AND column_name = 'status') THEN
    ALTER TABLE "shop_listings"
      ALTER COLUMN "status" DROP DEFAULT,
      ALTER COLUMN "status" TYPE shop_listings_status_enum
      USING CASE
        WHEN "status"::text IN ('active', 'sold', 'draft', 'removed') THEN "status"::text::shop_listings_status_enum
        ELSE 'active'::shop_listings_status_enum
      END,
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'shop_listings' AND column_name = 'mediaUrls') THEN
    ALTER TABLE "shop_listings"
      ...
      ALTER COLUMN "mediaUrls" TYPE json
      USING CASE
        WHEN "mediaUrls" IS NULL THEN '[]'::json
        ELSE "mediaUrls"::text::json
      END,
      ALTER COLUMN "mediaUrls" SET DEFAULT '[]'::json;
  END IF;
END $$;
--> statement-breakpoint
ALTER TABLE "shop_listings" ALTER COLUMN "sellerId" SET NOT NULL;
--> statement-breakpoint
ALTER TABLE "shop_listings" ALTER COLUMN "title" SET NOT NULL;
--> statement-breakpoint
ALTER TABLE "shop_listings" ALTER COLUMN "price" SET NOT NULL;
--> statement-breakpoint
...
--> statement-breakpoint
ALTER TABLE "shop_listings" ALTER COLUMN "condition" SET DEFAULT 'good';
--> statement-breakpoint
ALTER TABLE "shop_listings" ALTER COLUMN "condition" SET NOT NULL;
--> statement-breakpoint
...
--> statement-breakpoint
ALTER TABLE "shop_listings" ALTER COLUMN "status" SET DEFAULT 'active';
--> statement-breakpoint
ALTER TABLE "shop_listings" ALTER COLUMN "status" SET NOT NULL;
--> statement-breakpoint
